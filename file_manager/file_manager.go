...
/*
	有4个因素回影响磁盘的读写速度：
	1. 容量（磁盘能够存储的数据量），现在一个盘片的容量可以达到40G，容量越大（
	a.磁盘的数据密度就会增加：意味者相同物理空间中可以存储更多的数据，读写头需要在更小的区域内进行操作，磁头在一个轨道上可以读写更到数据），读写效率越高
	b.较大容量的磁盘通常可以使用更快的处理器，更大的缓存，更高速度的磁盘控制器
	c.较大容量的磁盘通常在存储数据时更好的进行数据的分布，避免了碎片化的问题，碎片化可能会导致小文件分布在不同的磁道上，降低读效率
	...
	我们把文件当作磁盘看待，按照区块为单位来对文件进行读写，OS会尽量把同一个文件的数据存储在磁盘的同一轨道中，或是距离近的轨道中
